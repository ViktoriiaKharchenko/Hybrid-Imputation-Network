...
def load_raw_data(data_dir, testing):
    def load_ith_pt_training(i):
        pt_with_na = pd.read_csv(data_dir / 'train_MCAR_80' / f'{i}.csv').values
        pt_ground_truth = pd.read_csv(data_dir / 'train_groundtruth_no_na' / f'{i}.csv').values
    ...
    def load_ith_pt_testing(i):
        pt = pd.read_csv(data_dir / 'test_MCAR_80' / f'{i}.csv').values
        pt_ground_truth_test = pd.read_csv(data_dir / 'test_groundtruth_no_na' / f'{i}.csv').values
        time_stamps = pt[:, 0]
        pt, pt_ground_truth_test = pt[:, 1:], pt_ground_truth_test[:, 1:]
        ...
        pt_ground_truth_test, missing_flag_gt = replace_nan_with_col_mean(pt_ground_truth_test)
        eval_mask = (~missing_flag_gt) & missing_flag  # 1: locs masked for eval
        eval_mask = eval_mask.astype(float)
        ...
        return {
            'pt_with_na': pt,
            'time_stamps': time_stamps,
            'pt_ground_truth': pt_ground_truth_test,
            'observed_mask': observed_mask,
            'eval_mask': eval_mask,
            'pt_max': ptmax,
            'pt_min': ptmin,
    ...
    if not testing:
        num_pts = len(list((data_dir / 'train_MCAR_80').glob('*.csv')))
        all_pts = [load_ith_pt_training(i) for i in range(1, num_pts+1)]
        return all_pts
    else:
        num_pts = len(list((data_dir / 'test_MCAR_80').glob('*.csv')))
